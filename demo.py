...
CONFIG_PATH = "/workspace/MultiMR/config/demo.yaml"
...
for split in hf_dataset:
    if split=="test":
        print(f"Building ID to content mapping for split: {split}")
        for entry in hf_dataset[split]:
            doc_id = entry['id'] 
            image_data = entry.get('image', None)
            text_data = entry.get('caption', None)  
...
H, W = image_size
padded_image = torch.zeros((3, H, W)) 
...
if ngpus > 0 and torch.cuda.is_available():
    print(f"Attempting to move FAISS index to {ngpus} GPU(s).")
    co = faiss.GpuMultipleClonerOptions()
    co.shard = True
    index_gpu = faiss.index_cpu_to_all_gpus(index_cpu, co=co, ngpu=ngpus)
    if hasattr(index_gpu, 'nprobe'):
        index_gpu.nprobe = 10 
        print(f"nprobe set to {index_gpu.nprobe} for GPU index.")
    ...
    if hasattr(FAISS_INDEX_READY, 'nprobe'):
        ...
        print(f"nprobe set to {FAISS_INDEX_READY.nprobe} for CPU index.")

...
class CLIPScoreFusion(nn.Module):
    def __init__(
        ...
        print(f"Loaded standard CLIP model: {model_name}")
    ...
    def get_tokenizer(self):
        def tokenizer_wrapper(texts: list[str], context_length=77):
                return self.tokenizer(texts, context_length=context_length, truncate=True)
...
class CLIPWeightFusion(nn.Module):
    def __init__(
        self, 
        model_name: str = "ViT-B/32",
        device="cpu", 
...
class CLIPMLPFusion(nn.Module):
    ...
    def fuse_embeddings(self, img_emb, txt_emb):
        concat = torch.cat([img_emb, txt_emb], dim=-1)  # shape: [B, 2 * dim]
        fused_emb = self.fusion_mlp(concat)  
        return fused_emb

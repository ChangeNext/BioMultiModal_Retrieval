...
# import pandas as pd # 데이터 핸들링 용이
...
#         # 예: 1: "Text-to-Image Retrieval", 2: "Image-to-Image Retrieval"
#     }
...
#         # 인덱스 생성 시 GPU 사용 여부 판단
#         ngpus = faiss.get_num_gpus()
        
#         if ngpus > 0:
#             print(f"Number of GPUs used for indexing: {ngpus}")
#             # GPU 인덱스를 먼저 생성하고 CPU로 옮겨서 저장
#             # IDMap 인덱스는 ID를 매핑하므로 "IDMap," 접두사를 사용합니다.
#             cpu_index = faiss.index_factory(
#                 faiss_config.dim,
#                 f"IDMap,{faiss_config.idx_type}", # "IDMap," 추가
#                 metric,
#             )
...
#             config.split, # 'test'
#             config.model_name, # ckpt_config.ckpt_name 대신 model_name
#             f"cand_{suffix_key}_index.faiss", # .faiss 확장자 사용
#         )
...
#     # 필요에 따라 실제 Query ID 변환 로직을 구현하세요.
#     # 현재는 단순히 문자열로 변환합니다.
#     return str(hashed_id)

# def unhash_did(hashed_id):
#     # 필요에 따라 실제 Document ID 변환 로직을 구현하세요.
#     # 현재는 단순히 문자열로 변환합니다.
#     return str(hashed_id)
...
#     # ID와 임베딩을 딕셔너리에 매핑하여 ID로 검색할 수 있게 합니다.
...
# def visualize_retrieval(query_embedding, retrieved_embeddings, retrieved_ids, k_val, plot_title, output_dir, file_prefix="retrieval_visualize"):
#     """
#     주어진 쿼리 임베딩과 검색된 상위 K개의 임베딩을 2D로 차원 축소하여 시각화하고 저장합니다.
#     """
#     # 모든 임베딩을 하나의 배열로 결합
#     # 쿼리 임베딩을 맨 앞에 추가하여 시각화 시 구분 가능하게 함
#     all_embeddings = np.vstack([query_embedding, retrieved_embeddings])

#     # 차원 축소 (PCA 또는 t-SNE)
#     # 데이터 포인트가 적으므로 PCA가 더 안정적일 수 있습니다.
#     if all_embeddings.shape[0] >= 2: # 최소 2개 이상의 점이 있어야 PCA/t-SNE 가능
#         if all_embeddings.shape[0] > 50: # t-SNE는 데이터가 많을 때 시간이 오래 걸림
...
#     # ID 추가 (쿼리는 Q-ID, 검색된 문서는 D-ID)
#     # 파일명 접두사에서 쿼리 ID를 가져옴 (예: Q_queryid_C...)
#     query_label = "Query"
#     try:
#         # file_prefix는 "Q<query_id>_C<suffix>_top<k>" 형태일 것으로 예상
#         q_id_part = file_prefix.split('_')[0][1:] # "Q" 다음의 ID 추출
#         query_label = f"Q-{q_id_part}"
#     except IndexError:
#         pass # 파싱 실패 시 기본값 사용

#     display_ids = [query_label] + [f"D-{_id} (R:{rank})" for _id, rank in zip(retrieved_ids, retrieved_ranks)]
#     df['ID'] = display_ids
...
#         # 첫 번째 (쿼리) 점은 색상 강조를 위해 다른 색으로 표시할 수도 있습니다.
...
#     # 파일 저장
#     file_name = f"{file_prefix}.png" # timestamp는 이미 file_prefix에 포함되어 있다고 가정하거나, file_prefix에서 추출
...
#                         continue
#                     query_embeddings_all = np.load(embed_query_path).astype('float32') # 모든 쿼리 임베딩 로드
...
#                         embed_query_path,
#                         cand_index_path,
#                         batch_size=retrieval_config.batch_size, # config에서 batch_size 사용
#                         num_cand_to_retrieve=k,
#                     )

#                     # --- 시각화 로직 시작 ---
#                     # 시각화할 쿼리의 인덱스 (예: 첫 번째 쿼리)
#                     viz_query_idx = 0 
#                     if len(hashed_query_ids) > viz_query_idx:
#                         selected_query_id_hashed = hashed_query_ids[viz_query_idx]
#                         selected_query_embedding = query_embeddings_all[viz_query_idx] # 로드된 모든 쿼리 임베딩에서 선택
#                         selected_retrieved_indices_hashed = retrieved_indices[viz_query_idx] # 검색된 문서의 해시된 ID들
                        
#                         # 시각화할 K 값 (예: 100)
#                         # config에서 설정하거나 고정값 사용
...
#                         # 검색된 문서의 실제 임베딩 가져오기 위해 원본 cand_embed 파일 로드
...
#                             # 검색된 상위 viz_k 개의 문서에 대해 실제 임베딩과 ID를 수집
#                             for idx_hashed in selected_retrieved_indices_hashed[:viz_k]:
#                                 unhashed_id = unhash_did(idx_hashed) # 해시된 ID를 원래 문자열 ID로 변환
...
#                                     f"(Top {viz_k} for Query ID: {unhash_qid(selected_query_id_hashed)})"
#                                 )
...
#                         }
#                         for metric in metric_recall_list:
#                             if recalls[metric]: # 데이터가 있을 때만 평균 계산
#                                 mean_recall_at_k = round(sum(recalls[metric]) / len(recalls[metric]), 4)
#                             else:
#                                 mean_recall_at_k = 0.0 # 계산할 데이터가 없으면 0으로 설정
...
#     # config 객체에 시각화 관련 설정이 없다면 기본값 추가
#     if not hasattr(config.retrieval_config, 'viz_k'):
#         config.retrieval_config.viz_k = 100 # 시각화할 top-k 값
#     if not hasattr(config.retrieval_config, 'batch_size'):
#         config.retrieval_config.batch_size = 8 # 검색 시 사용할 배치 크기
...
def visualize_retrieval(query_embedding, retrieved_embeddings, retrieved_ids, retrieved_ranks, k_val, plot_title, output_dir, file_prefix="retrieval_visualize"):
    ...
    query_label = "Query"
    try:
        # file_prefix는 "Q" 다음의 ID를 포함할 것으로 예상 (예: Q123_C...)
        q_id_part = file_prefix.split('_')[0][1:]
        query_label = f"Q-{q_id_part}"
    except IndexError:
        pass # 파싱 실패 시 기본값 사용

    # 여기서 retrieved_ranks를 사용하여 ID 옆에 랭크를 추가합니다.
            ...
            for q_suffix, q_mode_str in query_modalities.items():
                for c_suffix, c_mode_str in candidate_modalities.items():
                    ...
                    if len(hashed_query_ids) > viz_query_idx:
                        ...
                        retrieved_embeddings_for_viz = []
                        retrieved_ids_for_viz = []
                        retrieved_ranks_for_viz = [] # Add this list to store ranks
                        ...
                        if cand_id_to_embedding is not None:
                            ...
                            for rank, idx_hashed in enumerate(selected_retrieved_indices_hashed[:5]):
                                ...
                                if unhashed_id in cand_id_to_embedding:
                                    retrieved_embeddings_for_viz.append(cand_id_to_embedding[unhashed_id])
                                    retrieved_ids_for_viz.append(unhashed_id)
                                    retrieved_ranks_for_viz.append(rank + 1) # Ranks are 1-based
                                else:
                                    print(f"Warning: Original embedding not found for retrieved ID {unhashed_id}. Skipping.")
                            ...
                            if retrieved_embeddings_for_viz:
                                ...
                                visualize_retrieval(
                                    ...
                                    retrieved_embeddings=retrieved_embeddings_for_viz,
                                    retrieved_ids=retrieved_ids_for_viz,
                                    retrieved_ranks=retrieved_ranks_for_viz, # Pass the ranks here
                                    k_val=5,
                                    plot_title=plot_title,
                                    output_dir=viz_output_dir,
                                    file_prefix=f"Q{unhash_qid(selected_query_id_hashed)}_C{c_suffix}_top{viz_k}_{datetime.now().strftime('%H%M%S')}"
